...
// Fetch Projects
// ----------------------------------------
$projectsApi = "https://projects.thedeltagroup.co.in/wp-json/wl/v1/projects";
$projectsRes = fetchApiData($projectsApi);

$projects = $projectsRes['status'] ? $projectsRes['data'] : [];
if (!$projectsRes['status']) {
    echo "Project API Error: " . $projectsRes['error'];
}
?>
...
function buildUrl($page, $slug = '')
{
    ...
    // Project page
    if ($pageName === "property-details" && $slug) {
